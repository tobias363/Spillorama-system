...
| I14 | Popup-auto-show feiler ved manuell test selv om automatisert E2E passerer. `[BUY-DEBUG][client][TicketGrid.rebuild]` fyrer men ingen `showBuyPopup`-kall — én av 5 gate-conditions i `PlayScreen.update():693-702` blokkerer. Diagnose-log lagt til (`autoShowGate`-event med alle 5 conditions). Tobias-rapport 2026-05-13 etter pilot-test-fundament merget. | Implementasjons (under diagnose) | feat/playscreen-autoshow-gate-2026-05-13 (PR #1316) | 🔴 Under diagnose |
| I15 | **Re-entry til rom under pågående trekning blokkeres.** Tobias-rapport 2026-05-13: "etter at jeg starter spill går ut av lobbyen for deretter å gå inn igjen så kommer jeg ikke inn i rommet under en trekning, må vente til trekning er ferdig før jeg kan gå inn." Forventet atferd: spiller skal kunne re-joine rom når som helst og se aktuell state + sine kjøpte brett (read-only mid-runde). Kandidat root-cause: `room:join`-handler avviser når `gameStatus===RUNNING` med `ROOM_LOCKED_DURING_DRAW` el.l., ELLER klient blokkerer routing til play-screen fra lobby ved aktiv runde. | Implementasjons (under diagnose) | TODO | 🔴 Bekreftet av Tobias |
| I16 | **Plan-run lifecycle stuck-state etter test-cleanup.** Etter E2E-test-runs satt `app_game_plan_run.status='running'` mens `app_game1_scheduled_games.status='completed'`. Lobby returnerer `nextScheduledGame.scheduledGameId` pekende på terminal-runde → spiller-klient ser ingen joinable game (popup vises ikke). Tobias' manuelle test 1.5h etter test-suite → 1h diagnose. Root cause: `MasterActionService.stop()` kaster `ENGINE_FAILED` via `wrapEngineError` FØR `planRunService.finish()` rakk å kjøre. Fix: `Game1LobbyService.tryReconcileTerminalScheduledGame` auto-healer på lobby-poll — siste-position + terminal sched-game → auto-finish plan-run (idempotent, audit-actor `system:lobby-auto-reconcile`); ikke-siste position → hide scheduledGameId fra response. Fail-safe via DB-error-catch. 10 nye unit-tester. Se F-02. | Implementasjons | fix/plan-run-auto-reconcile-2026-05-13 | 🟡 PR pending |

---

| I15 | `game1:join-scheduled` (`apps/backend/src/sockets/game1ScheduledEvents.ts:288-365`) kaller `engine.joinRoom` direkte uten å sjekke for eksisterende player via `findPlayerInRoomByWallet` først. Spiller som navigerer tilbake til lobby (`returnToShellLobby`) og inn igjen mid-runde får `PLAYER_ALREADY_IN_ROOM` og lander på `Game1LobbyFallback`-overlay i stedet for pågående runde — fordi `detachSocket` (`BingoEngine.ts:3802`) beholder player-record (kun socketId nullstilles) av regulatoriske grunner (armed-state/lucky/forhåndskjøp), og `assertWalletNotAlreadyInRoom` (`BingoEngine.ts:4198-4211`) kaster når wallet allerede er bound. `room:join`/`room:create` har riktig re-attach-guard (`roomEvents.ts:372-397, 771-806`). Fix: la `findPlayerInRoomByWallet` + `attachPlayerSocket`-guard i `joinScheduledGame` rett før `engine.joinRoom`-call (speiler eksakt `room:join`-mønsteret). Repro-test: `tests/e2e/spill1-reentry-during-draw.spec.ts` (grønn etter fix). Diagnose-doc: `docs/architecture/REENTRY_BUG_DIAGNOSE_2026-05-13.md`. FRAGILITY F-05 dokumenterer at ALLE join-handler-paths må ha guard. | Implementasjons | fix/reentry-during-draw-2026-05-13 | 🟡 PR pending |
...
| 2026-05-13 | I15 (re-entry during draw) diagnostisert og reprodusert. Bug: `joinScheduledGame` i `game1ScheduledEvents.ts:288` mangler re-attach-guard som `room:create`/`room:join` har. Spillere som navigerer tilbake til lobby og inn igjen mid-runde får `PLAYER_ALREADY_IN_ROOM` og lander på `Game1LobbyFallback` i stedet for pågående runde. Implementasjons-bug (én funksjon, < 30 linjer fix). Test `spill1-reentry-during-draw.spec.ts` + diagnose-doc pushed på `feat/reentry-diagnose-2026-05-13`. Forventet 🔴 inntil fix lander. | reentry-diagnose agent |
| 2026-05-13 | I15 fix implementert. Re-attach-guard lagt til i `joinScheduledGame` (`game1ScheduledEvents.ts:297-336`) som speiler `room:join`-mønsteret eksakt: `findPlayerInRoomByWallet` → `attachPlayerSocket` → return existing playerId. 4 nye unit-tester (`game1ScheduledEvents.reconnect.test.ts`) + E2E `spill1-reentry-during-draw.spec.ts` PASSER (14.9s). Branch `fix/reentry-during-draw-2026-05-13`. FRAGILITY F-05 + PITFALLS §7.13 oppdatert med utvidet kontekst (PR #1218 fikset KUN delta-watcher; denne fikser initial-join-pathen). | reentry-fix agent |
