...
| I15 | `game1:join-scheduled` (`apps/backend/src/sockets/game1ScheduledEvents.ts:288-365`) kaller `engine.joinRoom` direkte uten å sjekke for eksisterende player via `findPlayerInRoomByWallet` først. Spiller som navigerer tilbake til lobby (`returnToShellLobby`) og inn igjen mid-runde får `PLAYER_ALREADY_IN_ROOM` og lander på `Game1LobbyFallback`-overlay i stedet for pågående runde — fordi `detachSocket` (`BingoEngine.ts:3802`) beholder player-record (kun socketId nullstilles) av regulatoriske grunner (armed-state/lucky/forhåndskjøp), og `assertWalletNotAlreadyInRoom` (`BingoEngine.ts:4198-4211`) kaster når wallet allerede er bound. `room:join`/`room:create` har riktig re-attach-guard (`roomEvents.ts:372-397, 771-806`). Fix: legg `findPlayerInRoomByWallet` + `attachPlayerSocket`-guard i `joinScheduledGame` rett før `engine.joinRoom`-call. Repro-test: `tests/e2e/spill1-reentry-during-draw.spec.ts`. Diagnose-doc: `docs/architecture/REENTRY_BUG_DIAGNOSE_2026-05-13.md`. | Implementasjons | TODO | 🔴 Bekreftet via test |
