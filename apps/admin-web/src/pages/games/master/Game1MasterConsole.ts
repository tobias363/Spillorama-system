...
/**
 * Render master-konsollen for et gitt spill. Kaller fetchGame1Detail +
 * setter opp polling. Stopper eksisterende polling før ny startes.
 */
export async function renderGame1MasterConsole(
  ...
  // Polling re-fetch'er hele detail-payload — enkel og trygg fallback for
  // socket-lag som ikke er wired i admin-web ennå.
  activePoll = setInterval(() => {
    void refresh(container, gameId);
  }, POLL_INTERVAL_MS);
  // Stop polling når container fjernes fra DOM.
...
async function refresh(container: HTMLElement, gameId: string): Promise<void> {
  try {
    ...
    // Hvis 401/403 → stopp polling (token utløpt).
