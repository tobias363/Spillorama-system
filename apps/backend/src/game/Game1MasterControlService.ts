 ...
 *        eller være ADMIN).
 *      - Validerer ingen ikke-bekreftet ekskludert hall (confirmExcludedHalls
 *        må inneholde alle hall-IDer med excluded_from_game=true for at master
 *        skal kunne starte selv om haller er ekskluderte — tvinger eksplisitt
 *        bekreftelse).
 ...
 *      - Master kan ikke ekskludere master-hallen selv (ville bryte invarianten).
 *      - UPDATE app_game1_hall_ready_status: excluded_from_game=true,
 *        excluded_reason.
 *      - Side-effekt: hvis spillet var i 'ready_to_start' men exclusion-en
 *        fjerner alle non-excluded ready-haller → status tilbake til
 *        'purchase_open' (scheduler-tick tar det i neste runde).
 *      - AuditLog: 'exclude_hall' med hallId + reason i metadata.
 *   3) includeHall({ gameId, hallId, actor })
 *      - Reverser exclusion. Kun gyldig i 'purchase_open' (ikke ready_to_start
 *        for å forhindre race).
 *      - AuditLog: 'include_hall' med hallId i metadata.
 *   4) pauseGame({ gameId, reason?, actor })
 *      - Gyldig i status='running'.
 *      - UPDATE status='paused'.
 *      - AuditLog: 'pause' med reason i metadata.
 *   5) resumeGame({ gameId, actor })
 *      - Gyldig i status='paused'.
 ...
 *     til BingoEngine (eksisterende draw-engine)" men BingoEngine er room-
 *     scoped, ikke scheduled-game-scoped. PR 3-scope begrenses derfor til
 *     scheduled-games-state-maskin + audit; faktisk draw-engine-wiring
 *     kommer i senere PR når room-code-mapping er definert.
 *   - Hall-scope (user må tilhøre master_hall_id) håndheves i route-laget.
 *   - Audit skrives i samme transaksjon som state-oppdateringen for å
 *     unngå drift. Feiler en av dem → begge rulles tilbake.
 *
 * AuditLog-actions (DB-constraint):
 *   start | pause | resume | stop | exclude_hall | include_hall | timeout_detected
 */
...
/**
 * Master-action whitelist. Matcher CHECK-constraint i
 * 20260428000200_game1_master_audit.sql.
 */
...
/**
 * Actor-kontekst. Route-laget bygger dette fra den autentiserte PublicAppUser
 * + oppslag av actor.hallId hvis de mangler hall (ADMIN har ofte null).
 */
export interface MasterActor {
  userId: string;
  /** Hall actor jobber fra. ADMIN kan settes til master-hall (for audit), eller 'ADMIN_CONSOLE'. */
  hallId: string;
...
export interface StartGameInput {
  gameId: string;
  /** Array of hallId's som master bekrefter eksplisitt å starte uten. */
...
/**
 * Kontrakt: Master-control mutator kjører i en transaksjon og returnerer
 * audit-raden + oppdatert game-status. Testene kan stub'e hele servicen.
 */
export class Game1MasterControlService {
  ...
  /**
   * Start et scheduled game. Gyldig fra:
   *  - status='ready_to_start'
   *  - status='purchase_open' hvis alle non-excluded haller er ready
   *
   * Master må eksplisitt bekrefte ekskluderte haller via
   * `confirmExcludedHalls` — array av hall-IDer som matcher aktuelle
   * excluded_from_game=true-rader. Dette forhindrer at master glemmer
   * å inkludere en hall som er blitt ekskludert underveis.
   */
  async startGame(input: StartGameInput): Promise<MasterActionResult> {
    return this.runInTransaction(async (client) => {
      ...
      // Pre-cond: status må være ready_to_start ELLER purchase_open med allReady.
      ...
      if (game.status === "purchase_open") {
        // Krav: minst én non-excluded hall må være ready.
      ...
      // Confirm excluded halls: alle excluded-rader må være bekreftet av master.
  ...
  /**
   * Ekskluder hall fra spill (teknisk feil). Gyldig i purchase_open eller
   * ready_to_start. Master-hall kan ikke ekskluderes (ville bryte invarianten).
   *
   * Hvis exclusion gjør at alle non-excluded ready-haller nå faller under
   * terskelen, vil scheduler-tick flippe status fra ready_to_start tilbake
   * til purchase_open ved neste runde.
   */
  async excludeHall(input: ExcludeHallInput): Promise<MasterActionResult> {
    ...
    return this.runInTransaction(async (client) => {
      ...
      // UPSERT ekskludering (rad kan mangle hvis hall aldri ble ready).
      await client.query(
      ...
      // Side-effekt: hvis game var i ready_to_start, sjekk om vi må rullere
      // tilbake til purchase_open (i tilfelle en ready-hall nå ble ekskludert
      // etter at all-ready var nådd — vi vil ikke la master starte uten å
      // re-verifisere tilstanden).
      ...
      // Hent oppdatert snapshot EFTER eksklusjon.
      const readyRows = await this.loadReadySnapshot(client, input.gameId);
  ...
  /**
   * Reverser eksklusjon. Kun gyldig i 'purchase_open' (ikke
   * 'ready_to_start' — der må master re-evaluere via scheduler-tick).
   */
   ...
   * game-stateovergang: skriver kun ny rad hvis det ikke allerede finnes en
   * timeout_detected-rad for dette game siden siste state-endring.
   *
   * Brukes av Game1ScheduleTickService.detectMasterTimeout når en game har
   * vært i 'ready_to_start'-status lenger enn timeout-terskelen uten at
  ...
  async recordTimeoutDetected(
    ...
    return this.runInTransaction(async (client) => {
      ...
      // Idempotens: ingen ny rad hvis det finnes en 'timeout_detected' etter
      // siste state-endring (updated_at). Bruker updated_at som proxy for
      // "state-epoke" — ikke perfekt men stabilt.
  ...
  /**
   * Detalj-view av et scheduled game + beregnet ready-status + siste
   * audit-hendelser. Brukes av master-konsoll GET-endpointet.
   */
  ...
  /**
   * Hent actor-rollens permission-sjekk: ADMIN har globalt scope;
   * HALL_OPERATOR/AGENT må tilhøre master-hallen.
   */
  ...
  private async runInTransaction<T>(
    ...
    try {
      ...
      try {
        ...
        // swallow rollback error — original will propagate.
      }
      throw err;
    } finally {
      client.release();
