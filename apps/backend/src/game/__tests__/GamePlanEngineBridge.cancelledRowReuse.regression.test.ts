 ...
 * gjenbruk (markert "Åpen" i PITFALLS_LOG.md per 2026-05-12).
 *
 * PITFALLS §4.4 symptom:
 *   Mark-ready feiler med `GAME_NOT_READY_ELIGIBLE: 'cancelled'` etter at
 *   runde har vært cancelled tidligere samme dag.
 *
 * Root cause beskrevet i PITFALLS_LOG:
 *   `createScheduledGameForPlanRunPosition` gjenbruker eksisterende rader
 *   på `(plan_run_id, plan_position)` uten status-filter.
 *
 * Fix-anbefaling:
 *   Filter `WHERE status NOT IN ('cancelled','finished')` på
 *   idempotency-lookup.
 *
 * Denne test-suiten:
 *   1. Forsetter bug-en strukturelt — hvis koden gjenbruker en cancelled
 *      rad, FEILER testen tydelig (regresjon når bug-en blir fikset).
 *   2. Tester begge scenarier:
 *      a) Cancelled rad finnes for (run, pos) → forventer at bridgen
 *         SPAWNER NY RAD (ikke gjenbruker). Hvis test feiler → bug-en er
 *         fortsatt åpen.
 ...
 *
 * NB om "todo"-flagg:
 *   Hvis bug-en er ÅPEN (per PITFALLS_LOG), markeres test-en med `todo:
 *   true` slik at det fanger oppmerksomhet uten å brekke CI. Når bug-en
 *   blir fikset, fjernes `todo`-flagget og test-en blir blokkerende.
 *
 * Status pr 2026-05-12: PITFALLS-entry sier åpen. Vi setter `todo: true`
 * på regresjons-test og forventer at den FEILER inntil fix lander.
 */
...
test(
  ...
  {
    skip: skipReason,
    // PITFALLS_LOG.md flagger denne som "Åpen" pr 2026-05-12. Vi setter
    // `todo: true` så test-en eksplisitt logger gap-en uten å bremse CI.
    // Når PITFALLS-entry endres til "Lukket", fjern todo-flagget.
    todo: "PITFALLS §4.4 åpen pr 2026-05-12 — bridge gjenbruker cancelled rad. Fjern todo når fix lander.",
    timeout: 30_000,
  },
  async () => {
    if (!PG_CONN) return;
...
test(
  ...
  {
    skip: skipReason,
    todo: "PITFALLS §4.4 åpen pr 2026-05-12 — bridge gjenbruker også completed-rad. Fjern todo når fix lander.",
    timeout: 30_000,
  },
  async () => {
    if (!PG_CONN) return;
