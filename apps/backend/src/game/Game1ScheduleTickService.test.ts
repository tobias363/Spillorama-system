...
test("detectMasterTimeout: 42P01 (tabell mangler) → returnerer tom liste", async () => {
  ...
  // Override pool.query for INSERT-kallet slik at det kaster 42P01.
  const origQuery = pool.query;
  pool.query = async (sql: string, params?: unknown[]) => {
    if (sql.includes("INSERT INTO") && sql.includes("master_audit")) {
      insertAttempts++;
