 ...
 *
 * Dekker:
 *   - startGame happy path fra ready_to_start og fra purchase_open+allReady
 *   - startGame avviser status-mismatch, non-ready halls, uconfirmed excluded
 *   - startGame avviser non-master actor
 *   - excludeHall happy path + master-hall-beskyttelse + status-rollback
 *   - includeHall happy path + status-restriksjon
 *   - pauseGame / resumeGame happy + error-states
 *   - stopGame happy + status-allowlist
 *   - recordTimeoutDetected idempotens
 *   - getGameDetail returnerer audit + ready-status + snapshot
 *   - audit-rad skrives med korrekt action + snapshot
 */
...
function createStubPool(responses: StubResponse[] = []): {
  ...
  // Preserve order but allow re-use: we don't splice since multiple matches
  // can hit in sequence (e.g. snapshot-load + update). Instead, we iterate
...
test("startGame happy path fra ready_to_start", async () => {
  const { pool, queries } = createStubPool([
    ...
    // loadReadySnapshot again before audit (not needed - check code path)
    // writeAudit
    {
  ...
  assert.ok(auditQuery, "audit query should have run");
...
test("excludeHall ruller ready_to_start tilbake til purchase_open", async () => {
  ...
  assert.equal(result.status, "purchase_open");
  // Verify rollback query was sent.
  ...
  assert.ok(rollback, "should roll back ready_to_start → purchase_open");
});
...
test("recordTimeoutDetected skriver audit-rad første gang", async () => {
  const { pool, queries } = createStubPool([
    ...
    {
      match: (s) =>
        s.includes("COUNT(*)") && s.includes("timeout_detected"),
      rows: [{ count: "0" }],
    },
    {
      match: (s) => s.includes("hall_id, is_ready, excluded_from_game"),
