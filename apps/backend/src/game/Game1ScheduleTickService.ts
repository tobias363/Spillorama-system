...
export class Game1ScheduleTickService {
   ...
   *
   * @param nowMs                  nåtid (injisert for test).
   * @param timeoutThresholdSeconds terskel i sekunder (default 900 = 15 min).
   * @returns                      { gameIds: string[] } — liste av game-IDer som var i timeout.
   */
  async detectMasterTimeout(
    ...
    // For hver kandidat, sjekk idempotens (timeout_detected-rad etter siste
    // state-endring). Vi skriver audit via direkte INSERT her for å holde
    // Game1ScheduleTickService fri for coupling mot Game1MasterControlService
    // (som trenger Pool + transaksjon).
    ...
    for (const g of candidates) {
      ...
      try {
        ...
        const code = (err as { code?: string } | null)?.code ?? "";
        if (code === "42P01") {
          // Audit-tabellen mangler (dev uten migrasjon) — skippe silent.
