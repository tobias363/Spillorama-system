 ...
 *     Body: { confirmExcludedHalls?: string[] }
 *     Permission: GAME1_MASTER_WRITE (ADMIN + HALL_OPERATOR + AGENT)
 *     Hall-scope: HALL_OPERATOR/AGENT må tilhøre game.master_hall_id.
 *
 *   POST /api/admin/game1/games/:gameId/exclude-hall
 *     Body: { hallId: string, reason: string }
 *     Permission: GAME1_MASTER_WRITE
 *
 ...
 *     Returnerer: { game, halls, auditRecent }
 *
 * Rolle-krav:
 ...
 *
 * Audit:
 *   - Skrives KUN via app_game1_master_audit (regulatorisk §11). Vi
 *     duplikerer IKKE til AuditLogService for master-actions — spec §3.9
 *     tilsier at master-audit er game1-spesifikk trail. Hall-scope-feil
 *     logges imidlertid via AuditLogService (parallelt med eksisterende
 *     ready-flow) slik at security-audit har forbidden-requests.
 */
...
export interface AdminGame1MasterRouterDeps {
  ...
  /**
   * Socket.io server for master-action-broadcast. Valgfritt fordi
   * tester kan mount router uten socket; i prod er det alltid satt.
   */
  io?: SocketServer;
}
...
/**
 * Bygg MasterActor-kontekst fra PublicAppUser. For ADMIN som mangler
 * hallId bruker vi 'ADMIN_CONSOLE' som placeholder — audit-raden må ha
 * en hallId. (Alternativt kan vi kreve at ADMIN spesifiserer hall i
 * payload, men det er overhead for den dominerende use-casen.)
 */
function buildActor(
  user: PublicAppUser,
...
export function createAdminGame1MasterRouter(
  ...
  /**
   * Hent master-hall-id for spillet før permission-check. Trenger dette
   * for å bygge riktig MasterActor + for hall-scope-validering.
   */
  ...
  router.post("/api/admin/game1/games/:gameId/start", async (req, res) => {
    ...
    try {
      ...
      if (error instanceof DomainError && error.code === "FORBIDDEN" && gameId) {
        // Ikke-autorisert master-action — logg forbidden-audit.
        try {
