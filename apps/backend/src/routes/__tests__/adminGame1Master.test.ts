 ...
 *   - PLAYER / SUPPORT er utestengt fra write-endepunkter
 *   - ADMIN aksepteres selv uten hallId
 *   - Happy path → 200 + service-kall + socket-broadcast
 *   - Validation: manglende reason, manglende body
 *   - GET returnerer game + halls (med navn) + auditRecent + allReady-flagg
 *   - Error-propagering: service kaster DomainError → 400 + error.code
 */
...
const otherOperator: PublicAppUser = {
  ...adminUser,
  id: "op-x",
  role: "HALL_OPERATOR",
  hallId: "hall-other",
...
test("POST /start — ADMIN happy path → 200 + audit-id + socket-broadcast", async () => {
  ...
  try {
    ...
    // Socket-broadcast globalt + til group-room.
...
test("POST /start — no auth header avvises med 401 (downstream from UNAUTHORIZED)", async () => {
  const ctx = await startServer();
  try {
    const res = await fetch(`${ctx.baseUrl}/api/admin/game1/games/g1/start`, {
      method: "POST",
...
test("POST /exclude-hall — mangler body → 400", async () => {
  const ctx = await startServer({ users: { "t-admin": adminUser } });
  try {
    const res = await fetch(`${ctx.baseUrl}/api/admin/game1/games/g1/exclude-hall`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
test("POST /pause — tom body aksepteres", async () => {
  ...
  try {
    const res = await post(ctx, "/api/admin/game1/games/g1/pause", "t-admin", {});
    assert.equal(res.status, 200);
    // Ingen reason satt når body er tom.
...
test("GET /:gameId — happy path returnerer game + halls (med navn) + audit + allReady", async () => {
  const ctx = await startServer({
    users: { "t-admin": adminUser },
    halls: {
