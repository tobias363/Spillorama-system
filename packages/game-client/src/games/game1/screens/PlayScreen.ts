...
export class PlayScreen extends Container {
  ...
  update(state: GameState): void {
    ...
    // brett som 20 kr fordi `state.entryFee` (room.gameVariant.entryFee
    // default 20) ble brukt som fallback. Lobby-ticket-config har
    // korrekt per-bong-pris (5 kr for billigste bong i Spill 1). Vi
    // prioriterer lobbyTicketConfig.entryFee først (server autoritativ),
    // og faller tilbake til state.entryFee kun hvis lobby-config mangler.
    const gridEntryFee =
