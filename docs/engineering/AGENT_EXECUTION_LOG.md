...
| 2026-05-14 | **PR #1431 (Lobby-API nextGame for finished plan-run, komplementært til PR #1422)** — Tobias-rapport 13:00 (samme dag som PR #1422 landet): Master-UI viser fortsatt "Start neste spill — Bingo" etter Bingo (position=1) ferdig. PR #1422 fixet DB-side (create-logikk advancer korrekt), MEN lobby-API returnerte `nextScheduledGame: null` ved finished plan-run → master-UI faller tilbake til default plan-items[0] (Bingo). **Fix:** `Game1LobbyService.getLobbyState` finished-branch advancer til `plan.items[currentPosition + 1]` når `currentPosition < items.length`; `GameLobbyAggregator.buildPlanMeta` advancer `positionForDisplay` så `catalogSlug` peker til neste plan-item. Nytt `Game1LobbyState.planCompletedForToday`-flag speiler `PLAN_COMPLETED_FOR_TODAY`-DomainError. Jackpot-override-lookup endret fra `String(planRun.currentPosition)` til `String(positionForDisplay)` for konsistens. **Tester:** 5 nye i `Game1LobbyService.test.ts` (finished+next-position, finished på position=7/13, finished på position=13/13 = planCompletedForToday=true, sanity for ikke-finished states) + 2 nye i `GameLobbyAggregator.test.ts` (finished mid-plan med 2 + 13 items). Alle 77 lobby-tester PASS, TypeScript strict clean. **Skill-update:** `spill1-master-flow/SKILL.md` v1.7.1 follow-up. **Pitfall-update:** PITFALLS_LOG §3.13. **Shared-type update:** `Spill1LobbyState.planCompletedForToday?: boolean` (optional for backwards-compat). | Fix-agent P (a79dcb2baa1a2bcf3) |
| 2026-05-14 | **OBS-10 Wallet-integrity-watcher** (`feat/wallet-integrity-watcher-2026-05-14`) — cron-driven sjekk som håndhever to invariants på wallet-DB ved hver kjøring: (I1) balance-sum: `wallet_accounts.balance ≡ SUM(CASE side WHEN 'CREDIT' THEN amount ELSE -amount END)` over `wallet_entries`; (I2) hash-chain link: `row.previous_entry_hash ≡ predecessor.entry_hash` per `account_id` siste 24t. Brudd → Linear-issue Urgent (1) med label `wallet-integrity` og 24t per-wallet_id dedup, fallback til Slack-webhook + disk. Read-only, idempotent, default DISABLED — Tobias aktiverer manuelt. Komplementært til nattlig `WalletAuditVerifier` (full SHA-256). **Levert:** 7 nye filer (`scripts/ops/wallet-integrity-watcher.sh`, `wallet-mismatch-create-linear-issue.sh`, `setup-wallet-integrity-cron.sh`, `scripts/__tests__/ops/wallet-integrity-watcher.test.sh`, `docs/operations/WALLET_INTEGRITY_WATCHER_RUNBOOK.md`). **Tester:** 48 PASS lokalt (Q1 + Q2 JSON-shaping pure-function, dedup state-file 24t-vindu, Linear DRY_RUN-composer, pre-flight DB-check, balance-sum CREDIT/DEBIT-semantikk, hash-chain genesis-detect, integration smoke mot lokal DB). **Smoke-test mot lokal DB:** fanget 25 reelle I1-mismatches (load-test-wallets seedet uten ledger — kjent test-fiksturtilstand) + 3 I2 hash-chain-brudd (over 24t-vindu) → bekreftet at watcher fungerer mot ekte schema (NB: `wallet_accounts.balance` er `GENERATED ALWAYS` fra `deposit_balance + winnings_balance`, ikke en kolonne man kan UPDATE-e direkte; `wallet_entries.side` er CREDIT/DEBIT med positiv `amount`, ikke signed). **Skill-updates:** `wallet-outbox-pattern` v1.4.0 ny seksjon "Wallet-integrity-watcher (OBS-10)"; `audit-hash-chain` ny seksjon "Komplementære integrity-watchers"; `health-monitoring-alerting` ny OBS-10-seksjon parallell til OBS-9. **Pitfall:** PITFALLS_LOG §2.9 ny entry "Wallet integrity-check må kjøres cron, ikke kun on-demand" (indeks: §2 fra 8 → 9, total fra 95 → 96). **Tobias-direktiv 2026-05-14:** "Vi må fange wallet-mismatch og hash-chain-brudd så raskt at vi har sjanse til å forensicse hva som skjedde." Watcher gir < 1t MTTD vs nightly verifier (24t worst-case). **Mønster gjenbrukt:** OBS-9 db-perf-watcher-skeleton (Linear-issue script-shape, launchd/crontab setup, dedup state-file, DRY_RUN). **Lessons:** Agent-prompt sa skjema var `app_wallets(id, balance)` + `app_wallet_entries(wallet_id, amount signed)` — det stemmer IKKE. Faktisk: `wallet_accounts` + `wallet_entries` med `account_id`+`side`-enum og positiv amount. Verifisere DB-schema med `\d` FØR query-skriving sparer 20+ min. | Agent (wallet-integrity-watcher, a4dbd6...) |
| 2026-05-14 | **Synthetic Spill 1 bingo-runde-test (R4-precursor, BIN-817 forløper)** — Tobias-direktiv 2026-05-14: "Vi trenger ALLEREDE NÅ et synthetic end-to-end-test som verifiserer at en hel bingo-runde fungerer feilfritt." Etablert `scripts/synthetic/` med 4 moduler: `invariants.ts` (pure-compute evaluators for I1-I6), `api-client.ts` (typed fetch-wrapper med structured ApiError), `socket-client.ts` (Socket.IO observer for draw:new + room:update + pattern:won), `spill1-round-bot.ts` (CLI bot med `--mode=local|ci|dry-run`). Plus bash-wrapper `spill1-round-runner.sh` med pre-flight (backend /health + RESET_TEST_PLAYERS_TOKEN-sjekk) som eksporterer rapport til `/tmp/synthetic-spill1-YYYY-MM-DDTHH:MM:SS.md`. **Seks invarianter (I1-I6):** Wallet-konservering, Compliance-ledger, Hash-chain, Draw-sequence consistency, Idempotency, Round-end-state. **Tester:** 59 vitest unit-tester i `scripts/__tests__/synthetic/spill1-round-bot.test.ts` — dekker alle 6 evaluators + ApiClient med mocked fetch + bot helpers + dry-run smoke-test. Alle 59 PASS, TypeScript strict-mode clean (via `scripts/synthetic/tsconfig.json` + `package.json` for ESM). **Lessons learned:** (a) Pure-compute invariant-evaluators er lett-testbare og audit-vennlige — Lotteritilsynet kan re-kjøre på snapshot-fil. (b) Bot-en spør IKKE DB direkte — bruker `/api/_dev/debug/round-replay/<id>` (PR #1424) som autoritativ snapshot-kilde. Holder bot-en dekoblet fra DB-skjema. (c) `--dry-run` flag gjør CI-smoke-test mulig under 5 sek uten wallet-mutering. (d) Hash-chain (I3) er WARN inntil dev-endpoint legges til — bot har ikke DB-tilgang. **Skill-updates:** `casino-grade-testing/SKILL.md` v1.2.0, `live-room-robusthet-mandate/SKILL.md` v1.3.0, `spill1-master-flow/SKILL.md` v1.9.0. **Pitfall-update:** PITFALLS_LOG §6.18 (test må kjøres FØR pilot, ikke etter). **Runbook:** `docs/operations/SYNTHETIC_BINGO_TEST_RUNBOOK.md` (full operatør-guide med PASS/FAIL/WARN-tolking + per-invariant feilsøkings-tabell). **npm-scripts:** `test:synthetic` (root) + `test:synthetic-spill1` (apps/backend). **Forbudt-listen overholdt:** ingen endringer i `apps/backend/src/game/` (bot er ekstern observatør), ingen DB-mutering utenom officielle endpoints, ingen secrets committet. | synthetic-test-agent (aa2cc3afbfe693cab) |
