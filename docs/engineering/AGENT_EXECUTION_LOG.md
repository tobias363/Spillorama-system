...
| 2026-05-14 | **Auto-return-til-lobby etter runde-end (BUG, PR #1420)** — Tobias-rapport 2026-05-14 09:54 etter runde 330597ef ferdig: WinScreen viste 1 700 kr-gevinst korrekt, men "Forbereder rommet..."-spinner hang evig. Bruker MÅTTE klikke "Tilbake til lobby" manuelt. Fix i `Game1EndOfRoundOverlay.ts`: `MAX_PREPARING_ROOM_MS = 15_000` max-timeout med forced auto-return via `onBackToLobby` (samme path som manuell klikk). De siste 2 sekundene (`RETURNING_TO_LOBBY_PREVIEW_MS = 2_000`) byttes teksten til "Returnerer til lobby..." for synlig overgang. Sentry-breadcrumb `endOfRoundOverlay.autoReturnFallback`. Idempotent — cancelles av `markRoomReady`, manuell klikk, `hide()`. Reconnect-resilient via `elapsedSinceEndedMs`. Erstatter eldre 30s "Venter på master"-tekst-swap (PR #1006) som kun byttet tekst uten å trigge redirect — død kode fjernet. 7 nye unit-tester + 28 eksisterende grønne, alle 890 game1-tester PASS. **Lessons learned:** Klient-state-transisjoner kan ALDRI rely on backend-events alene — alltid ha timeout-fallback for live-UX. Tidligere 30s text-swap var stopgap, ikke fix. **Skill-update:** `live-room-robusthet-mandate/SKILL.md` v1.2.0 — ny seksjon "Etter-runde auto-return til lobby". **Pitfall-update:** PITFALLS_LOG §7.19 ny entry. | Fix-agent (auto-return) |
