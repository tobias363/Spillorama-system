# ADR-0013 — Per-spiller broadcast-strippet payload for perpetual rooms (Wave 3b)

**Status:** Accepted
**Dato:** 2026-05-06
**Deciders:** Tobias Haugen (Wave 3b — Anthropic Claude Opus 4.7, 1M context)
**Konsulterer:** —

> **Migrering 2026-05-08:** Migrert fra `docs/decisions/ADR-011-per-recipient-broadcast-perpetual-rooms.md` til ny struktur. Innhold uendret.
...
### Nøytrale
- Game1 berøres ikke — full payload beholdes der.

## Alternativer vurdert

### Alternativ A: Path A i audit (minimal) — bare strip `players[]`
Avvist:
- Sparer kun ~300 KB av 314 KB.
- Mister muligheten til å eliminere `tickets`/`marks`/`preRoundTickets`/`luckyNumbers` per-spiller-records.

### Alternativ B: Per-spiller diff-events (ingen room:update for Spill 2/3)
Deferred to Fase 3:
- Krever refactor av klient-state-management for å applye delta-eventer atomisk + flere socket-events pr. mutasjon.
- Stripping gir 99% av besparelsen til 1% av kompleksiteten.

### Alternativ C: Binary protocol (protobuf + gzip-compression)
Deferred to Fase 3:
- Casino-grade industri-norm men krever stor klient-side endring + protobuf-build-step.
- Stripping gir oss pilot-readiness uten det.
...
Skills som bør oppdateres: `spill2-perpetual-loop`, `spill3-phase-state-machine`,
`live-room-robusthet-mandate`.

## Referanser

- ADR-0002 (perpetual-room-model) — driver-kontekst
- ADR-0012 (batched-mass-payout) — søsterkontroll i samme Wave 3
- Audit: `docs/architecture/SPILL2_3_CASINO_GRADE_AUDIT_2026-05-05.md` §6.1 (TOP 5)
