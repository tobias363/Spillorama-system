# ADR-0012 — Batched parallel mass-payout for Spill 2/3 (Wave 3a)

**Status:** Accepted
**Dato:** 2026-05-06
**Deciders:** Tobias Haugen (Wave 3a — perf-engineering)
**Konsulterer:** —

> **Migrering 2026-05-08:** Migrert fra `docs/decisions/ADR-011-batched-mass-payout.md` til ny struktur. Innhold uendret.
...
### Alternativ A: Behold sequential-pathen
Avvist:
- 50-sek-blokkering av draw-tick gir kaskaderende timeout-errors
- Ikke akseptabelt på pilot-skala

### Alternativ B: Single Postgres-tx med UNNEST + INSERT
Vurdert som fremtidig forbedring (se Open questions). For nå: krever wallet-adapter API-endring og
DB-skjema-endring. Pre-allokering gir tilstrekkelig speedup uten det.

### Alternativ C: Eventuell konsistens (best-effort payout, retry senere)
Avvist:
- Spillere forventer å se gevinst innen sekunder
- Bryter casino-grade-prinsipp (deterministisk payout)
...
Skills som bør oppdateres: `wallet-outbox-pattern`, `spill2-perpetual-loop`, `spill3-phase-state-machine`,
...
- ADR-0002 (perpetual-room-model) — denne ADR-en bygger på perpetual-loop-modellen
- ADR-0003 (system-actor) — mass-payout-events bruker `actorType: "SYSTEM"`
- ADR-0004 (hash-chain-audit) — parallel I/O-batches kompatible med hash-chain
- ADR-0011 (casino-grade-observability) — nye metrics integreres i eksisterende stack
- `docs/architecture/SPILL2_3_CASINO_GRADE_AUDIT_2026-05-05.md` §3.1
