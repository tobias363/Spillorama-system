...
| 2026-05-17 | v1.9.0 — Fase B.2 av ADR-0024 follow-up: branch-cleanup. Ny `scripts/cleanup-merged-branches.sh` håndterer lokale + remote-branches (1 476 + 999 akkumulert). Per-branch klassifisering: MERGED (ancestor av main), SQUASH-MERGED (matcher merged-PR head via `gh pr list --state merged`-cache), FRESH (< --min-age), OPEN-PR (beholdes), WORKTREE (beholdes), CURRENT, PROTECTED (main/backup/recovery/restore — aldri), UNMERGED (manuell). DRY-RUN BY DEFAULT, `--apply`/`--yes`/`--remote`/`--all`/`--min-age`/`--json`. Empirisk DRY-RUN 2026-05-17: 1 340 branches trygt slettbare (532 lokal + 808 remote). PITFALLS §11.27 ny fallgruve. Bash 3.2-kompatibel (lessons fra Fase B fix-up). |
