...
| 2026-05-17 | v1.8.0 — Fase B av ADR-0024 follow-up: lokal cleanup-scripts for worktree+stash-baggage akkumulert (400+ worktrees, 178+ stashes). Nye `scripts/cleanup-merged-worktrees.sh` + `scripts/cleanup-stale-stashes.sh` med DRY-RUN BY DEFAULT, per-item Y/N-bekreftelse i `--apply`-mode, JSON-output, og bash 3.2-kompatibel. Worktree-verdikter: SAFE/LOCKED-S/ORPHANED/UNSAFE_*/CURRENT/MAIN. Stash-kategorier: AUTO-BACKUP/AGENT-LEFTOVER/MERGED-BRANCH/FRESH/EXPLICIT-KEEP/UNCLEAR. UNSAFE-verdikter slettes ALDRI. PM_SESSION_END_CHECKLIST Trinn 10 (valgfri). PITFALLS §11.26 ny fallgruve. |
| 2026-05-17 | v1.9.0 — Fase B.2 (PR #1565): `scripts/cleanup-merged-branches.sh` for lokal/remote branch-cleanup med gh-PR squash-merge-cache. 1 489 branches slettet på første kjøring. |
| 2026-05-17 | v1.10.0 — Fase B.3 av ADR-0024 follow-up: `cleanup-merged-worktrees.sh` utvidet med gh-PR squash-merge-cache (samme mønster som Fase B.2). Klassifiserer 116 ekstra worktrees som LOCKED-S i stedet for LOCKED-UNSAFE, fordi `git merge-base --is-ancestor` returnerer false for squash-merged branches. Optional gh/jq-avhengighet — hopp over hvis ikke tilgjengelig (faller tilbake til is-ancestor). Empirisk: 396 → 32 worktrees etter `--apply --include-locked` (~10 GB disk frigjort). §11.26 utvidet med Fase B.3-mønster. |
